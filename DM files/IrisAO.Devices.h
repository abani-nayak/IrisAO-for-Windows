/***************************************************************************************************************\
    ...
    FILE:       IrisAO.Devices.h  Copyright © 2009-2013 Iris AO, Inc. All Rights Reserved.
  
    PURPOSE:    Provides driver and mirror interface communications support.
...
#ifndef IRISAO_DEVICES_INCLUDED /* VERSION 1.0.2.5 */
#define IRISAO_DEVICES_INCLUDED
...
__declspec(dllimport) MirrorCommands MirrorInitSettings;

__declspec(dllimport) MirrorCommands MirrorSendSettings;

__declspec(dllimport) void MirrorCommand (MirrorHandle mirror, MirrorCommands command);

__declspec(dllimport) MirrorHandle MirrorConnect (SerialNumber mirror, SerialNumber driver, HardwareDisabled disabled);

__declspec(dllimport) bool MirrorIterate (MirrorHandle mirror, SegmentNumber &segment);

__declspec(dllimport) MirrorHandle MirrorRelease (MirrorHandle mirror);
...
__declspec(dllimport) void GetMirrorPosition (MirrorHandle mirror, SegmentNumber segment, MirrorPosition* position);

__declspec(dllimport) void SetMirrorPosition (MirrorHandle mirror, SegmentNumber segment, float z, float xgrad, float ygrad);

__declspec(dllimport) void SetModalPosition (MirrorHandle mirror, CoefficientNumber number, float value);

#ifdef __cplusplus
}
#endif

#endif // IRISAO_DEVICES_INCLUDED
